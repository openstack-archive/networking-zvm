...
class zvmNeutronAgent(object):
    ...
    def _treat_devices_added(self, devices):
        ...
        for userid, nic_list in nics_info.items():
            LOG.debug("Adding NICs for %(user)s, info: %(nic)s",
                      {'user': userid, 'nic': nic_list})
            self._utils.add_nics_to_direct(self._zhcp_node, userid, nic_list)
