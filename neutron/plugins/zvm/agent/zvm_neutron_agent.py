...
class zvmNeutronAgent(object):
    ...
    def _check_xcat_version(self):
        ...
        while (not version_ok):
            LOG.warning("WARNING: the xcat version communicating with is "
    ...
    def _treat_vif_port(self, port_id, network_id, network_type,
        ...
        LOG.info("Update port for node:%s" % node)
        if admin_state_up:
    ...
    def xcatdb_daemon_loop(self):
        ...
        while True:
            try:
                ...
                LOG.error("Lost connection to xCAT. %s", e)
                connect = False
            except Exception as e:
                LOG.exception("error in xCAT DB query loop: %s", e)
    ...
    def _init_xcat_mgt(self):
        ...
        try:
            ...
            LOG.error("Failed to create xCAT management network: %s" %
                      err.format_message())
            raise err
